 ...
 * [getDefaultAnkiDroidDirectory] if the user has not yet decided where the collection is stored.
 *
 * Called during app startup: reading the collection path
 * ([CollectionHelper.getCurrentAnkiDroidDirectory]) does not set a default.
 *
...
fun ensureCollectionPathSet(context: Context) {
    ...
    val defaultPath = getDefaultAnkiDroidDirectory(context).absolutePath
    Timber.i("collection path set to default: %s", defaultPath)
