 ...
 */
class SortOrderBottomSheetFragment : BottomSheetDialogFragment(R.layout.fragment_bottom_sheet_list) {
     ...
     */
    private inner class SortOrderHolderAdapter(
        private val columns: List<ColumnUiModel>,
        ...
        override fun onBindViewHolder(
            ...
            setupClickListeners(column, holder)
            ...
            holder.binding.sortStandard.isEnabled = true
            holder.binding.sortReverse.isEnabled = true
            holder.binding.noSortOrder.isEnabled = true

            holder.binding.sortStandard.clearColorFilter()
            holder.binding.sortReverse.clearColorFilter()
            holder.binding.noSortOrder.clearColorFilter()

            holder.binding.divider.isVisible = false

            // show a divider after the last active column
        ...
        private fun setupClickListeners(
            ...
            fun ImageView.setupListener(
                ...
                val sortType =
                    when (column) {
                        is ColumnUiModel.NoOrdering -> SortType.NoOrdering
                        is ColumnUiModel.AnkiColumn ->
                            SortType.CollectionOrdering(
                                key = column.key,
                                reverse = requireNotNull(reversed) { "" },
                            )
                    }
            ...
            holder.binding.sortReverse.setupListener("sortReversed", reversed = true)
            holder.binding.sortStandard.setupListener("sortStandard", reversed = false)
            holder.binding.noSortOrder.setupListener("noSortOrder", reversed = null)
        }
