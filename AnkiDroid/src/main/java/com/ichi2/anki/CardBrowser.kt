...
open class CardBrowser :
    ...
    fun transformBrowserRow(id: CardOrNoteId): BrowserRow {
        return CollectionManager.getBackend().browserRowForId(id.cardOrNoteId)
    }
    ...
    private suspend fun selectedCardIds(): List<CardId> = viewModel.queryAllSelectedCardIds()
    ...
    private fun setupFlows() {
        ...
        fun onSelectedRowsChanged(rows: BrowserRowCollection) = onSelectionChanged()
...
//        }
    ...
    private fun openNoteEditorForCurrentlySelectedNote() = launchCatchingTask {
        try {
            ...
            openNoteEditorForCard(viewModel.querySelectedCardIdAtPosition(index = 0))
        } catch (e: Exception) {
    ...
    private fun exportSelected() {
        val (type, selectedIds) = viewModel.getSelectionExportData() ?: return
    ...
    private fun onResetProgress() {
        ...
        launchCatchingTask {
            val dialog = ForgetCardsDialog.newInstance(viewModel.queryAllSelectedCardIds())
    ...
    private suspend fun editSelectedCardsTags(selectedTags: List<String>, indeterminateTags: List<String>) = withProgress {
        val selectedNids = viewModel.queryAllSelectedNoteIds().distinct()
        undoableOp {
            val selectedNotes = selectedNids
     ...
     */
    private fun updateCardsInList(@Suppress("UNUSED_PARAMETER") cardIds: List<CardId>) {
        updateList()
    }
