...
open class CardBrowser :
    ...
    @VisibleForTesting
    fun onTap(id: CardOrNoteId) = launchCatchingTask {
        if (viewModel.isInMultiSelectMode) {
            viewModel.toggleRowSelection(id)
        } else {
            // load up the card selected on the list
            val destination = viewModel.queryEditNoteDestination(id)
            // HACK: set the current card id. This should no longer be necessary
            openNoteEditorForCard(destination.cardId)
        }
    }
    ...
    private fun setupFlows() {
        ...
        fun onSelectedRowsChanged(rows: Set<CardOrNoteId>) = onSelectionChanged()
