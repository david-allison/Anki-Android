...
object InitialActivity {
    ...
    fun getStartupFailureType(initializeAnkiDroidDirectory: () -> Boolean): StartupFailure? {
        ...
        // checked before any collection access: opening the collection would throw
        // a StorageNotConfiguredException, which is expected and not worth a crash report
        if (CollectionHelper.storageDecision() != StorageDecision.Decided) {
            Timber.i("storage is undecided")
